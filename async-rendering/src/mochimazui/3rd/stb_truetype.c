...
static int stbtt_BakeFontBitmap(const unsigned char *data, int offset,  // font location (use offset=0 for plain .ttf)
...
static void stbtt_GetBakedQuad(stbtt_bakedchar *chardata, int pw, int ph,  // same data as above
...
static void stbtt_MakeCodepointBitmapSubpixel(const stbtt_fontinfo *info, unsigned char *output, int out_w, int out_h, int out_stride, float scale_x, float scale_y, float shift_x, float shift_y, float embolden, int codepoint);
...
static void stbtt_GetCodepointBitmapBoxSubpixel(const stbtt_fontinfo *font, int codepoint, float scale_x, float scale_y, float shift_x, float shift_y, float embolden, int *ix0, int *iy0, int *ix1, int *iy1);
...
static void stbtt_MakeGlyphBitmapSubpixel(const stbtt_fontinfo *info, unsigned char *output, int out_w, int out_h, int out_stride, float scale_x, float scale_y, float shift_x, float shift_y, float embolden, int glyph);
static void stbtt_GetGlyphBitmapBoxSubpixel(const stbtt_fontinfo *font, int glyph, float scale_x, float scale_y,float shift_x, float shift_y, float embolden, int *ix0, int *iy0, int *ix1, int *iy1);
...
int stbtt_InitFont(stbtt_fontinfo *info, const unsigned char *data2, int fontstart)
{
   ...
   for (i=0; i < numTables; ++i) {
      ...
      //printf("%04x %04x\n",ttUSHORT(data+encoding_record),ttUSHORT(data+encoding_record+2));
...
void stbtt_GetGlyphBitmapBoxSubpixel(const stbtt_fontinfo *font, int glyph, float scale_x, float scale_y,float shift_x, float shift_y, float embolden, int *ix0, int *iy0, int *ix1, int *iy1)
{
   int x0,y0,x1,y1;
   ...
   if (ix0) *ix0 =  STBTT_ifloor((x0-embolden) * scale_x + shift_x);
   if (iy0) *iy0 = -STBTT_iceil ((y1+embolden) * scale_y + shift_y);
   if (ix1) *ix1 =  STBTT_iceil ((x1+embolden) * scale_x + shift_x);
   if (iy1) *iy1 = -STBTT_ifloor((y0-embolden) * scale_y + shift_y);
}

void stbtt_GetCodepointBitmapBoxSubpixel(const stbtt_fontinfo *font, int codepoint, float scale_x, float scale_y, float shift_x, float shift_y, float embolden, int *ix0, int *iy0, int *ix1, int *iy1)
{
   stbtt_GetGlyphBitmapBoxSubpixel(font, stbtt_FindGlyphIndex(font,codepoint), scale_x, scale_y,shift_x,shift_y, embolden, ix0,iy0,ix1,iy1);
}
...
static void stbtt__rasterize(stbtt__bitmap *result, stbtt__point *pts, int *wcount, int windings, float scale_x, float scale_y, float shift_x, float shift_y, int off_x, int off_y, int invert, void *userdata)
{
   ...
   stbtt__edge *e;
   int n,i,j,k,m;
   int vsubsample = 5;//15;//result->h < 8 ? 15 : 5;
...
static void stbtt_Embolden(stbtt_vertex *vertices,int n,float lg);
void stbtt_MakeGlyphBitmapSubpixel(const stbtt_fontinfo *info, unsigned char *output, int out_w, int out_h, int out_stride, float scale_x, float scale_y, float shift_x, float shift_y, float embolden, int glyph)
{
   int ix0,iy0;
   ...
   stbtt_Embolden(vertices,num_verts,embolden);
   stbtt_GetGlyphBitmapBoxSubpixel(info, glyph, scale_x, scale_y, shift_x, shift_y, embolden, &ix0,&iy0,0,0);
   gbm.pixels = output;
   gbm.w = out_w;
   gbm.h = out_h;
   ...
   if (gbm.w && gbm.h)
      stbtt_Rasterize(&gbm, 0.2f, vertices, num_verts, scale_x, scale_y, shift_x, shift_y, ix0,iy0, 1, info->userdata);

   STBTT_free(vertices, info->userdata);
}

void stbtt_MakeCodepointBitmapSubpixel(const stbtt_fontinfo *info, unsigned char *output, int out_w, int out_h, int out_stride, float scale_x, float scale_y, float shift_x, float shift_y, float embolden, int codepoint)
{
   stbtt_MakeGlyphBitmapSubpixel(info, output, out_w, out_h, out_stride, scale_x, scale_y, shift_x, shift_y, embolden, stbtt_FindGlyphIndex(info,codepoint));
}
...
typedef stbtt_fontinfo* FT_Face;

EXPORT FT_Face FTW_LoadFont(const char* a,int sz){
	stbtt_fontinfo* info;
	int ofs=stbtt_GetFontOffsetForIndex((const u8*)a,0);
	if(ofs<0){return NULL;}
	info=(stbtt_fontinfo*)STBTT_malloc(sizeof(stbtt_fontinfo),NULL);
	if(!stbtt_InitFont(info,(const u8*)a,ofs)){
    	STBTT_free(info,NULL);
    	return NULL;
	}
    return info;
}
...
EXPORT int FTW_SetPixelSize(FT_Face ft,int h,int* metrics){
	float scale=stbtt_ScaleForPixelHeight(ft,(float)(h&0x3fffffff));
	int ascent=0,descent=0,lineGap=0;
	stbtt_GetFontVMetrics(ft, &ascent, &descent, &lineGap);
	ft->pixel_ascent=(int)ceilf((float)ascent*scale);
	metrics[0]=(h&0x3fffffff);
	metrics[1]=ft->pixel_ascent;
	ft->scale=scale;
	return h;
}
	...
	for(i=0;i<n;i++){
		if(vertices[i].type==STBTT_vmove){
			...
			for(j=0;j<np;j++){
				int j_prev=(j==0?np-1:j-1);
				int j_next=(j==np-1?0:j+1);
				float dx_next=(float)Ps[j_next*2+0]-(float)Ps[j*2+0];
				float dy_next=(float)Ps[j_next*2+1]-(float)Ps[j*2+1];
				float dx_prev=(float)Ps[j*2+0]-(float)Ps[j_prev*2+0];
				float dy_prev=(float)Ps[j*2+1]-(float)Ps[j_prev*2+1];
				float lg_next=sqrtf(dx_next*dx_next+dy_next*dy_next);
				float lg_prev=sqrtf(dx_prev*dx_prev+dy_prev*dy_prev);
				float Nx,Ny,lg_N;
				if(lg_next>0.f){lg_next=1.f/lg_next;}else{lg_next=0.f;}
				if(lg_prev>0.f){lg_prev=1.f/lg_prev;}else{lg_prev=0.f;}
				Nx=-((dy_next*=lg_next)+(dy_prev*=lg_prev));
				Ny=(dx_next*=lg_next)+(dx_prev*=lg_prev);
				//lg_N=sqrtf(Nx*Nx+Ny*Ny);
				lg_N=Ny*dx_next-Nx*dy_next;
				//thresholding
				if(lg_N*lg_N>(Nx*Nx+Ny*Ny)*(1.f/65536.f)){lg_N=lg/lg_N;}else{lg_N=0.f;}
				if(!lg_next||!lg_prev){lg_N=0.f;}
				Ns[j*2+0]=Nx*lg_N;
				Ns[j*2+1]=Ny*lg_N;
			}
			Ns[np*2+0]=Ns[0];
			...
			for(i2=i;i2<n;i2++){
				...
				if(vertices[i2].type==STBTT_vcurve){
					vertices[i2].cx+=(short)STBTT_ifloor(Ns[p*2+0]+0.5f);
					vertices[i2].cy+=(short)STBTT_ifloor(Ns[p*2+1]+0.5f);
					p++;
				}
				vertices[i2].x+=(short)STBTT_ifloor(Ns[p*2+0]+0.5f);
				vertices[i2].y+=(short)STBTT_ifloor(Ns[p*2+1]+0.5f);
				p++;
			}
		}
	}
...
EXPORT u8* FTW_GetCharacter(FT_Face ft,int embolden,int ch,int* ret){
	//its AA is likely slower, but shouldn't be too much
	int advance=0,lsb=0,x0=0,y0=0,x1=0,y1=0;
	int sz_needed=0;
	//int i=0;/*,cmax=1;*/
	float emboldenf=(float)embolden/ft->scale*(1.f/200.f);
	ch&=0xffffff;
	stbtt_GetCodepointHMetrics(ft, ch, &advance, &lsb);
	stbtt_GetCodepointBitmapBoxSubpixel(ft, ch, ft->scale,ft->scale,0.f,0, emboldenf,&x0,&y0,&x1,&y1);
	sz_needed=(x1-x0)*(y1-y0);
	if(g_buf_size<sz_needed){
		if(g_buf){STBTT_free(g_buf,ft->userdata);}
		g_buf_size=sz_needed;
		g_buf=(u8*)STBTT_malloc(sz_needed,ft->userdata);
	}
	//printf("%08x %d %d %d %d %f\n",g_buf,x0,x1,y0,y1,ft->scale);
	stbtt_MakeCodepointBitmapSubpixel(ft, g_buf, x1-x0,y1-y0, x1-x0, ft->scale,ft->scale,0.f,0.f, emboldenf,ch);
	/*return the bitmap dimensions and location -- x,y,w,h, pitch,dx*/
	ret[0]=x0;
	ret[1]=ft->pixel_ascent+y0;
	ret[2]=x1-x0;
	ret[3]=y1-y0;
	ret[4]=x1-x0;
	ret[5]=(int)ceilf((float)advance*ft->scale);
    return g_buf;
}

EXPORT float FTW_GetAscent(FT_Face ft,int h){
	float scale=stbtt_ScaleForPixelHeight(ft,(float)(h&0x3fffffff));
	int ascent=0,descent=0,lineGap=0;
	stbtt_GetFontVMetrics(ft, &ascent, &descent, &lineGap);
...
float embolden, int ch,float* ret){
	float scale=stbtt_ScaleForPixelHeight(ft,h);
	int advance=0,lsb=0,x0=0,y0=0,x1=0,y1=0;
	int sz_src=0,sz_needed=0;
	float emboldenf=embolden/scale*(1.f/10000.f);
	int ascent=0,descent=0,lineGap=0;
	...
	y1_pixel=(y1-1)/y_aa;y1_pixel+=(y1_pixel>>31)+1;
	if(y0==y1){y1_pixel=y0_pixel;}
	himg=y1_pixel-y0_pixel;
	wimg=x1-x0+x_htaps*2;
	sz_src=(((x1-x0)*(y1-y0)+3)&-4);
	sz_needed=sz_src+(x1-x0)*himg*4;
	if(g_buf_size<sz_needed){
		if(g_buf){STBTT_free(g_buf,ft->userdata);}
		g_buf_size=sz_needed;
		g_buf=(u8*)STBTT_malloc(sz_needed,ft->userdata);
	...
	stbtt_MakeCodepointBitmapSubpixel(ft, g_buf, x1-x0,y1-y0,x1-x0, (float)x_aa*scale,(float)y_aa*scale,0.f,0.f, emboldenf,ch);
	{
		//filter it
		int i, j, k;// x, y;
		int wsrc=x1-x0,hsrc=y1-y0;
		//y filter first
		for(i=0;i<himg;i++){
			int y=(y_aa>>1)+y_aa*(i+y0_pixel)-y0;
			int addr=(sz_src>>2)+i*wsrc;
			for(j=0;j<wsrc;j++){
				float acc=0.f;
				//int acci;
				for(k=-y_htaps;k<=y_htaps;k++){
					if((unsigned int)(y+k)<(unsigned int)hsrc){
						acc+=(float)(g_buf[(y+k)*wsrc+j])*y_filter[k+y_htaps];
					}
				}
				//acci=(int)acc;if(acci<0){acci=0;}else if(acci>255){acci=255;}
				//g_buf[addr+j]=acci;
				((float*)g_buf)[addr+j]=acc;
			}
		}
		//x filter (more costly)
		for(i=0;i<himg;i++){
			int addr_src=(sz_src>>2)+i*wsrc;
			int addr_tar=i*wimg;
			for(j=0;j<wimg;j++){
				int x=j-x_htaps;
				float acc=0.f;
				int acci;
				for(k=-x_htaps;k<=x_htaps;k++){
					if((unsigned int)(x+k)<(unsigned int)wsrc){
						acc+=((float*)g_buf)[addr_src+x+k]*x_filter[k+x_htaps];
					}
				}
				acci=(int)acc;if(acci<0){acci=0;}else if(acci>255){acci=255;}
				g_buf[addr_tar+j]=acci;
			}
		}
	}
	/*return the bitmap dimensions and location -- x,y,w,h, x_draw,dx*/
	ret[0]=((float)x0-x_htaps)/(float)x_aa;
	ret[1]=(float)ascent*scale+(float)y0_pixel;
	ret[2]=(float)wimg;
	ret[3]=(float)himg;
	ret[4]=(float)(wimg)/(float)x_aa;
	ret[5]=(float)advance*scale;
    return g_buf;
}

EXPORT float FTW_GetGlyphAdvance(FT_Face ft,float h,int id){
	float scale=stbtt_ScaleForPixelHeight(ft,h);
	int advance=0,lsb=0;
	stbtt_GetGlyphHMetrics(ft, id, &advance, &lsb);
	return (float)advance*scale;
}

EXPORT float FTW_GetCharAdvance(FT_Face ft,float h,int ch){
	float scale=stbtt_ScaleForPixelHeight(ft,h);
	int advance=0,lsb=0;
	stbtt_GetCodepointHMetrics(ft, ch, &advance, &lsb);
	return (float)advance*scale;
...
EXPORT float FTW_GetKerning(FT_Face ft,float h,int ch1, int ch2){
	float scale=stbtt_ScaleForPixelHeight(ft,h);
	return (float)stbtt_GetCodepointKernAdvance(ft,ch1,ch2)*scale;
}
